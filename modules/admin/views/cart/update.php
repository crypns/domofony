...
//); ?>
        ...
        <?= $form->field($model, 'general_count')->textInput(['type' => 'number', 'min'=>'0']) ?>
        <!-- end attribute -->
			<!-- attribute `general_cost` -->
        <?= $form->field($model, 'general_cost')->widget(\kartik\money\MaskMoney::className(), [
            'pluginOptions' => [
                'prefix' => '₴ ',
            ]
        ]); ?>
        ...
        <?= $form->field($model, 'status_order')->dropDownList($model::STATUSES,
            [
                'prompt' => Yii::t('cruds', 'Select'),
                'disabled' => (isset($relAttributes) && isset($relAttributes['parent_category'])),
                'value' => $model->status_order,

                'options' => [
                    isset($parentCategory) ? $parentCategory->id : '' => ['selected' => true],
                ],
                'id' => 'parent-category',
            ]
        ) ?>
